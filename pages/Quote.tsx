...
import { Shield, Smartphone, Lightbulb, Zap, HelpCircle, CheckCircle, ArrowRight, ArrowLeft } from 'lucide-react';
...
const SERVICE_OPTIONS = [
  { id: 'Cibersegurança', icon: <Shield className="w-6 h-6 mb-3" />, desc: 'Proteção e auditoria de sistemas' },
  { id: 'Desenvolvimento Web/Mobile', icon: <Smartphone className="w-6 h-6 mb-3" />, desc: 'Plataformas seguras' },
  { id: 'Consultoria IT', icon: <Lightbulb className="w-6 h-6 mb-3" />, desc: 'Estratégia e planeamento' },
  { id: 'Transformação Digital', icon: <Zap className="w-6 h-6 mb-3" />, desc: 'Modernização de infraestrutura' },
    ...
    return (
      <>
        ...
        <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center px-4 py-20 text-center">
          ...
          <h1 className="font-orbitron text-3xl md:text-5xl text-white font-bold mb-4">Pedido Recebido!</h1>
          <p className="font-rajdhani text-slate-400 text-lg mb-8 max-w-md">
            A nossa equipa comercial e de engenharia irá analisar os seus requisitos. Respondemos em 48 horas úteis.
          </p>
          <Link to="/" className="px-8 py-3 bg-cyan-400 text-slate-950 font-bold uppercase tracking-widest hover:bg-white transition-colors">
            Voltar ao Início
          </Link>
        </div>
      </>
    ...
    <>
      ...
      <div className="min-h-screen bg-slate-950 pt-24 pb-20 px-4 flex flex-col items-center">
        ...
        <div className="text-center mb-12">
          ...
          <p className="font-rajdhani text-slate-400">Configure as necessidades da sua infraestrutura.</p>
        </div>
              ...
              {[1, 2, 3, 4].map((step) => (
                <div 
                  ...
                  className={`h-1.5 flex-1 rounded-full transition-colors duration-300 ${
                    currentStep === step ? 'bg-cyan-400' : 
                    currentStep > step ? 'bg-white' : 'bg-slate-800'
                  }`}
                />
              ))}
            </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            {state.step === 1 && (
              <div className="animate-in fade-in slide-in-from-right-4 duration-300">
                <h2 className="text-xl text-white font-bold mb-6 font-rajdhani">Qual é o tipo de serviço principal?</h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                  {SERVICE_OPTIONS.map((service) => (
                    ...
                    >
                      <div className={state.serviceType === service.id ? 'text-cyan-400' : 'text-slate-500'}>
                        {service.icon}
                      </div>
            ...
            {state.step === 2 && (
              <div className="animate-in fade-in slide-in-from-right-4 duration-300 space-y-8">
                <div>
                  <h2 className="text-xl text-white font-bold mb-6 font-rajdhani">Dimensão da Empresa</h2>
                ...
                <div>
                  <h2 className="text-xl text-white font-bold mb-6 font-rajdhani">Urgência do Projeto</h2>
                  <div className="grid grid-cols-2 gap-3">
                    {URGENCIES.map(urg => (
                      <button
                        key={urg}
            ...
            {state.step === 4 && (
              <div className="animate-in fade-in slide-in-from-right-4 duration-300 space-y-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div>
                    <label className="block text-xs font-mono text-slate-400 mb-2 uppercase">Nome Completo *</label>
                    <input 
                      required type="text"
                  ...
                  <div>
                    <label className="block text-xs font-mono text-slate-400 mb-2 uppercase">Empresa *</label>
                    <input 
                      required type="text"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div>
                    <label className="block text-xs font-mono text-slate-400 mb-2 uppercase">Email Profissional *</label>
                    <input 
                      required type="email"
                  ...
                  <div>
                    <label className="block text-xs font-mono text-slate-400 mb-2 uppercase">Telefone (Opcional)</label>
                    <input 
                      type="tel"
                ...
                <div>
                  <label className="block text-xs font-mono text-slate-400 mb-2 uppercase">Descrição do Projeto *</label>
                  <textarea 
                    required rows={4}
            ...
            <div className="pt-6 mt-8 border-t border-slate-800 flex justify-between">
              ...
              {state.step < 4 ? (
                ...
                <button
                  type="submit"
                  disabled={loading || isLimited || !state.name || !state.company || !state.email || !state.description}
