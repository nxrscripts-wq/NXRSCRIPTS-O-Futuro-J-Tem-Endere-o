...
const FAQItem: React.FC<{ question: string; answer: string }> = ({ question, answer }) => {
  ...
  return (
    <div className="border-b border-nxr-border">
      ...
      >
        <span className={`text-lg font-medium transition-colors ${isOpen ? 'text-nxr-primary' : 'text-white group-hover:text-nxr-primary'}`}>
          {question}
        </span>
        <span className={`ml-4 text-nxr-primary transition-transform duration-300 ${isOpen ? 'rotate-180' : ''}`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 mb-24">
          ...
          <div>
            ...
            <div className="mt-12 bg-slate-900 border border-slate-800 p-6 rounded-sm">
              <p className="text-xs text-slate-500 font-mono mb-2">ESTADO DO SISTEMA DE COMUNICAÇÕES</p>
              <div className="flex items-center space-x-2">
                <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></div>
                <span className="text-white text-sm font-bold">OPERACIONAL</span>
              </div>
              <p className="text-slate-400 text-sm mt-2">
                Todos os canais encriptados estão ativos. O tempo médio de resposta é de &lt; 2 horas.
              </p>
            </div>
          </div>
          ...
          <div className="bg-nxr-panel p-8 border border-nxr-border relative">
            ...
            {status === 'success' ? (
              <div className="text-center py-12">
                ...
                <p className="text-slate-400 mt-2">
                  ...
                  {category && <br />}
                  <span className="text-xs text-nxr-primary font-mono mt-2 block">Categorizado por IA como: {category}</span>
                </p>
                <button
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div>
                  <label htmlFor="subject" className="block text-sm font-medium text-slate-400 mb-2">Interesse no Produto / Assunto</label>
                  <input
                    type="text"
                    id="subject"
                    name="subject"
                ...
                <div>
                  ...
                  >
                    {status === 'analyzing' ? 'A Encriptar e Enviar...' : 'Iniciar Contacto Seguro'}
                  </button>
                  {isLimited && (
            ...
            {/* Corner Accent */}
            <div className="absolute top-0 right-0 w-0 h-0 border-t-[40px] border-r-[40px] border-t-transparent border-r-nxr-primary/20"></div>
          </div>
        </div>
