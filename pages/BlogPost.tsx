    ...
    return (
      <div className="min-h-screen bg-slate-950 pt-32 pb-24 flex justify-center items-center">
        <div className="animate-pulse flex flex-col items-center">
          ...
          <div className="text-cyan-400 font-mono text-sm tracking-widest uppercase">A decifrar dados...</div>
        </div>
      </div>
    );
  }
    ...
    <>
      ...
      <div className="min-h-screen bg-slate-950 pt-24 pb-24">
        <article className="max-w-3xl mx-auto px-4 sm:px-6">
          ...
          <Link to="/blog" className="inline-flex items-center text-slate-400 hover:text-cyan-400 font-mono text-xs uppercase tracking-widest mb-10 transition-colors">
          ...
          <header className="mb-10">
            ...
            <div className="flex flex-wrap items-center gap-6 text-sm text-slate-400 font-mono border-y border-slate-800 py-4">
              <div className="flex items-center">
                ...
                {post.published_at ? new Date(post.published_at).toLocaleDateString('pt-PT', { day: 'numeric', month: 'long', year: 'numeric' }) : 'N/A'}
              </div>
              <div className="flex items-center">
          ...
          <footer className="mt-16 pt-8 border-t border-slate-800">
            {post.tags && post.tags.length > 0 && (
              <div className="flex flex-wrap items-center gap-2 mb-10">
                ...
                {post.tags.map(tag => (
                  <span key={tag} className="px-3 py-1 bg-slate-900 border border-slate-700 text-slate-300 text-xs rounded-full">
                    #{tag}
                  </span>
                ))}
              </div>
            ...
            <Link to="/blog" className="inline-flex items-center px-6 py-3 bg-cyan-400 text-slate-950 font-bold uppercase tracking-widest text-sm hover:bg-white transition-colors">
              <ArrowLeft className="w-4 h-4 mr-2" />
