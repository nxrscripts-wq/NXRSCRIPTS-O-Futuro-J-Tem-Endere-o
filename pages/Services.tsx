...
import { ArrowUpRight, ShieldCheck, ScanEye, Code2, Server, Activity, Cloud, Globe, Smartphone, Network, ArrowRight } from 'lucide-react';
...
const ProcessStep: React.FC<{ number: string; title: string; description: string }> = ({ number, title, description }) => (
  <div className="relative p-6 border-l-2 border-nxr-border hover:border-nxr-primary transition-colors duration-300">
    <div className="absolute -left-[11px] top-6 w-5 h-5 bg-nxr-dark border-2 border-nxr-primary rounded-full"></div>
    <span className="text-4xl font-bold text-slate-800 absolute top-4 right-4 select-none">{number}</span>
...
const Services: React.FC = () => {
  return (
    <>
    <SEOHead page={SEO_PAGES.services} jsonLd={[organizationSchema(), ...SERVICES.map(s => serviceSchema(s.title, s.description, s.title))]} />
    <div className="pt-20 sm:pt-16 pb-24 min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center mb-20">
          <div>
            <h3 className="text-nxr-primary font-mono text-sm uppercase tracking-widest mb-2">Como Trabalhamos</h3>
            <h2 className="text-3xl font-bold text-white mb-6">O Protocolo de Segurança NXR</h2>
            <p className="text-slate-400 mb-8 leading-relaxed">
              Adotamos uma abordagem sistemática e militarizada para proteger os seus ativos. Desde a avaliação inicial até à monitorização contínua, o nosso processo garante que nenhuma vulnerabilidade passe despercebida.
            </p>
            <Link to="/contact" className="inline-flex items-center text-white hover:text-nxr-primary font-bold transition-colors">
        ...
        <div className="p-8 bg-gradient-to-r from-nxr-panel to-slate-900 border border-nxr-border rounded-sm text-center">
          <h3 className="text-2xl font-bold text-white mb-4">Soluções Empresariais Personalizadas</h3>
          <p className="text-slate-400 mb-8 max-w-2xl mx-auto">
            Precisa de uma arquitetura de segurança à medida? Os nossos engenheiros desenham soluções personalizadas para os setores governamental, financeiro e da saúde.
          </p>
          <Link to="/contact" className="px-6 py-3 bg-white text-nxr-dark font-bold hover:bg-nxr-primary transition-colors">
