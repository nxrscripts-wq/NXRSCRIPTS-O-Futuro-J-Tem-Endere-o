...
export const LeadModal: React.FC<LeadModalProps> = ({
  ...
  const getCategoryColor = (category: string) => {
    ...
    if (cat.includes('sales') || cat.includes('venda')) return 'text-cyan-400 bg-cyan-400/10 border-cyan-400/30';
    if (cat.includes('support') || cat.includes('suporte')) return 'text-blue-400 bg-blue-400/10 border-blue-400/30';
    if (cat.includes('partner') || cat.includes('parceir')) return 'text-green-400 bg-green-400/10 border-green-400/30';
    return 'text-slate-400 bg-slate-400/10 border-slate-400/30';
  };

  const getStatusColor = (status: LeadStatus) => {
    switch (status) {
        case 'NEW': return 'text-cyan-400 bg-cyan-400/10 border-cyan-400/30';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="flex items-start justify-between p-6 border-b border-slate-800">
          <div>
            <h2 id="modal-title" className="text-2xl font-bold text-white mb-1">
              {lead.name} <span className="text-slate-500 font-normal">{lead.company && `— ${lead.company}`}</span>
            </h2>
            <div className="flex items-center gap-3 mt-3">
              <span className={`px-2.5 py-0.5 rounded text-xs font-medium border ${getCategoryColor(lead.category)}`}>
                {lead.category || 'Geral'}
              </span>
              
              <div className="relative inline-block text-left group">
                  <span className={`inline-flex items-center px-2.5 py-0.5 rounded text-xs font-medium border cursor-pointer ${getStatusColor(lead.status)}`}>
                      {lead.status}
        ...
        <div className="p-6 overflow-y-auto flex-1 custom-scrollbar space-y-6">
          ...
          {/* Contact Info */}
          <div>
            <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-3">Informação de Contacto</h3>
            <div className="flex gap-4">
              <a href={`mailto:${lead.email}`} className="flex items-center gap-2 text-sm text-cyan-400 hover:text-cyan-300 transition-colors">
          ...
          {/* Original Message */}
          <div>
            <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-3">Mensagem Original</h3>
            <div className="bg-slate-800 rounded-lg p-4 text-slate-300 font-mono text-[13px] whitespace-pre-wrap leading-relaxed">
              {lead.message || <span className="italic text-slate-500">Sem mensagem original.</span>}
            </div>
          </div>
          ...
          {/* Internal Notes */}
          <div>
            <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-3">Notas Internas</h3>
            <textarea
