...
export default defineConfig(({ mode }) => {
    ...
    return {
      ...
      plugins: [
        react(),
        viteImagemin({
          plugins: { jpg: imageminMozjpeg({ quality: 80 }), png: imageminOptipng({ optimizationLevel: 5 }) },
          makeWebp: { plugins: { jpg: imageminWebp({ quality: 80 }), png: imageminWebp({ quality: 80 }) } },
        }),
        sitemap({
