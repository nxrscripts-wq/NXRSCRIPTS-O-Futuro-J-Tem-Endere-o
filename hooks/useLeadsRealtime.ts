...
export function useLeadsRealtime() {
  ...
  const isNewLead = useCallback((id: string) => recentLeadIds.has(id), [recentLeadIds]);
  ...
  useEffect(() => {
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'leads' }, payload => {
        ...
        console.warn('[Realtime] Lead change:', payload.eventType, payload.new);
        ...
        if (payload.eventType === 'INSERT' && payload.new) {
          ...
          queryClient.setQueryData<Lead[]>(['leads'], old =>
            old
              ? [optimisticLead, ...old.filter(l => l.id !== optimisticLead.id)]
              : [optimisticLead]
          );
      ...
      .subscribe(status => {
        if (status === 'SUBSCRIBED') {
          setIsConnected(true);
          console.warn('[Realtime] Leads channel connected');
        }
        if (status === 'CLOSED') {
          setIsConnected(false);
          console.warn('[Realtime] Leads channel closed');
        }
